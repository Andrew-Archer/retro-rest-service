...
let DashBoard = /** @class */ (function() {
    ...
    DashBoard.prototype.createDashBoard = function() {
        ...
        this.listBoards.forEach((board) => {
            let cardBoard = new CardBoard(board.name, board.columns, '22/03/2022', this.contentCards, board.id);
        ...
        modal.addEventListener('click', (e) => {
          ...
          if (target.closest('.ok')) {
				...
				if (isVal) {
					const board = new Board(name.value, arrColumns, this.parentEl, this.listBoards.length + 1);
					board.createBoard();
					name.value = '';
				}
          }
...
let CardBoard = /** @class */ (function() {
  ...
  CardBoard.prototype.createCardBoard = function() {
        ...
        textDateSpan.textContent = this.date;
        ...
        this.cardBoardDiv.addEventListener('click', (e) => {
            ...
            if (!e.target.closest('.card-board-delete')) {
                const board = new Board(this.name, this.columns, this.parentEl, this.idBoard);
                board.createBoard();
            }
        })
  }
...
var Board = /** @class */ (function() {
  function Board(name, columns, parentEl, idBoard) {
        this.name = name;
        this.columns = columns;
        ...
        // document.location.href = '?id=' + this.idBoard;
  }
  ...
  Board.prototype.createBoard = function() {
      ...
      let btnAddButton = createElementWithClasses('button', ['btn', 'btn-add']);
      ...
      btnColumnButton.addEventListener('click', (e) => {
          ...
          let column = new Column('Новая колонка', [], this.main, this.listColors[i]);
          column.createColumn();
      ...
      this.columns.forEach(columnEl => {
          let column = new Column(columnEl.name, columnEl.cards, this.main, this.listColors[i]);
          column.createColumn();
          if (i < this.listColors.length - 1) i++;
          else i = 0;
      });
...
var Column = /** @class */ (function() {
  function Column(name, cards, parentEl, color) {
      this.name = name;
  ...
  Column.prototype.createColumn = function() {
      ...
      let nameDiv = createElementWithClasses('div', ['name', 'border-' + this.color]);
      ...
      nameSpan.addEventListener('click', (e) => {
          e.preventDefault();
          nameDiv.removeChild(nameSpan);      
          nameDiv.appendChild(divEdit);
      ...
      this.cards.forEach(card => {
          let cardEl = new Card(card.title, cardsDiv, this.color);
          cardEl.createCard();
      });
      ...
      btnAddCard.addEventListener('click', (e) => {
          e.preventDefault();
          let card = new Card('Новая карточка', cardsDiv, this.color);
          card.createCard();
      })
...
var Card = /** @class */ (function() {
  function Card(title, parentEl, color) {
      this.title = title;
      this.parentEl = parentEl;
      this.svgPathD = {
      ...
      this.card = createElementWithClasses('div', ['card', 'color-' + color]);
      this.color = color;
  }
  ...
  Card.prototype.createCard = function() {
      ...
      this.card.appendChild(feedbackDiv);
      ...
      cardTextLikeSpan.textContent = '0';
      ...
      cardTextCommentSpan.textContent = '0';
      ...
      contentDiv.addEventListener('click', (e) => {
          e.preventDefault();
          this.card.removeChild(contentDiv);
          this.card.removeChild(feedbackDiv);
          this.card.appendChild(editCardDiv);
          this.card.classList.add('border2-' + this.color, 'edit');
          this.card.classList.remove('color-' + this.color);
          ...
          btnDone.addEventListener('click', (e) => {
              ...
              this.card.removeChild(editCardDiv);
              this.card.appendChild(contentDiv);
              this.card.appendChild(feedbackDiv);
              this.card.classList.remove('border2-' + this.color, 'edit');
              this.card.classList.add('color-' + this.color);
          });
...
let parsBoard = JSON.parse(JSON.stringify(jsonBoard));
//.base - элемент к которому привязываем приложение
let base = document.querySelector('.base');
//Далее создаем конструктор Board, который дальше отрисовывает приложение
const urlId = new URL(document.location).searchParams.get('id');
if (urlId > 0 && urlId <= parsBoard.board.length) {
    const board = new Board(parsBoard.board[urlId-1].name, parsBoard.board[urlId-1].columns, base, parsBoard.board[urlId-1].id);
    board.createBoard();
} else {
    let board = new DashBoard(parsBoard.board, base);
